...
const calculateTotalFuelForMass = (fuelMass: number): number => {
  let result = fuelMass;
  let additional = calculateFuelRequiredForMass(fuelMass);

  while (additional > 0) {
    result += additional;
    additional = calculateFuelRequiredForMass(additional);
  }
  return result;
};
...
const calculateTotalFuelRequired = () =>
  masses
    .map(calculateFuelRequiredForMass)
    .map(calculateTotalFuelForMass)
    .reduce(sum, 0);
